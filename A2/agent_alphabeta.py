...
def alphabeta_max_limit(board, curr_player, alpha, beta, heuristic_func, depth_limit):
    ...
    # Checking if it is a terminal state or if limit is 0
    ...
    for move in moves:
        next_board = play_move(board, curr_player, move)
        _, value = alphabeta_min_limit(next_board, get_opponent(curr_player), alpha, beta, heuristic_func, depth_limit)
...
def alphabeta_min_limit(board, curr_player, alpha, beta, heuristic_func, depth_limit):
    ...
    for move in moves:
        next_board = play_move(board, curr_player, move)
        _, value = alphabeta_max_limit(next_board, get_opponent(curr_player), alpha, beta, heuristic_func, depth_limit)
...
def alphabeta_max_limit_caching(board, curr_player, alpha, beta, heuristic_func, depth_limit, cache):
    ...
    # Checking if it is a cached state
    if (hash(board), curr_player) in cache:

        # Getting all the cached information
        (cached_best_move, cached_best_value, cached_alpha,
         cached_beta, cached_depth_limit) = cache[(hash(board), curr_player)]

        # Check if the cached state has deep enough depth limit
        if cached_depth_limit >= depth_limit and cached_alpha <= alpha and cached_beta >= beta:
            return cached_best_move, cached_best_value
    ...
    for move in moves:
        next_board = play_move(board, curr_player, move)
        _, value = alphabeta_min_limit_caching(next_board, get_opponent(curr_player), alpha, 
                                               beta, heuristic_func, depth_limit, cache)
    ...
    cache[(hash(board), curr_player)] = (best_move, best_value, alpha, beta, depth_limit + 1)
    return best_move, best_value
...
def alphabeta_min_limit_caching(board, curr_player, alpha, beta, heuristic_func, depth_limit, cache):
    ...
    for move in moves:
        next_board = play_move(board, curr_player, move)
        _, value = alphabeta_max_limit_caching(next_board, get_opponent(curr_player), alpha, 
                                               beta, heuristic_func, depth_limit, cache)
